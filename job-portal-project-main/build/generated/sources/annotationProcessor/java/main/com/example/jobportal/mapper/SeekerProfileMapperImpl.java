...
@Component
public class SeekerProfileMapperImpl implements SeekerProfileMapper {

    @Override
    public SeekerProfile fromDTO(SeekerProfileRequest seekerProfileRequest) {
        if ( seekerProfileRequest == null ) {
            return null;
        }

        SeekerProfile seekerProfile = new SeekerProfile();

        seekerProfile.setSeekerId( seekerProfileRequest.getSeekerId() );
        seekerProfile.setFirstName( seekerProfileRequest.getFirstName() );
        seekerProfile.setLastName( seekerProfileRequest.getLastName() );
        seekerProfile.setMajor( seekerProfileRequest.getMajor() );
        byte[] cv = seekerProfileRequest.getCv();
        if ( cv != null ) {
            seekerProfile.setCv( Arrays.copyOf( cv, cv.length ) );
        }
    ...
    @Override
    public SeekerProfileResponse toDTO(SeekerProfile seekerProfile) {
        if ( seekerProfile == null ) {
            return null;
        }

        SeekerProfileResponse seekerProfileResponse = new SeekerProfileResponse();

        seekerProfileResponse.setSeekerId( seekerProfile.getSeekerId() );
        seekerProfileResponse.setFirstName( seekerProfile.getFirstName() );
        seekerProfileResponse.setLastName( seekerProfile.getLastName() );
        seekerProfileResponse.setMajor( seekerProfile.getMajor() );
        seekerProfileResponse.setUser( userToUserResponse( seekerProfile.getUser() ) );
        seekerProfileResponse.setEducationDetails( educationDetailListToEducationDetailResponseList( seekerProfile.getEducationDetails() ) );
        seekerProfileResponse.setExperienceDetails( experienceDetailListToExperienceDetailResponseList( seekerProfile.getExperienceDetails() ) );

        return seekerProfileResponse;
    }

    @Override
    public List<SeekerProfileResponse> toDTOs(List<SeekerProfile> seekerProfiles) {
        if ( seekerProfiles == null ) {
            return null;
        }

        List<SeekerProfileResponse> list = new ArrayList<SeekerProfileResponse>( seekerProfiles.size() );
        for ( SeekerProfile seekerProfile : seekerProfiles ) {
            list.add( toDTO( seekerProfile ) );
        }
    ...
    @Override
    public void mapUpdateRequestToEntity(SeekerProfile seekerProfile, SeekerProfileRequest seekerProfileRequest) {
        if ( seekerProfileRequest == null ) {
            return;
        }
